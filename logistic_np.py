...
def normalize_per_pixel(train_x, test_x):
    ...
    for i in range(train_x.shape[0]):
        train_x[i] = np.divide(np.subtract(train_x[i],mean_per_pix),std_per_pix)
    for i in range(test_x.shape[0]):
        test_x[i] = np.divide(np.subtract(test_x[i],mean_per_pix),std_per_pix)
...
def test(y_hat, test_y):
    ...
    #precision = TP/(TP+FP)
    precision = np.sum(np.logical_and(y_hat>0.5,test_y==1))/np.sum(y_hat>0.5)
    #recall = TP/P
    recall = np.sum(np.logical_and(y_hat>0.5,test_y==1))/np.sum(test_y==1)
