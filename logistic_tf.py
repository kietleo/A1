...
if __name__ == "__main__":
    ...
    w = tf.Variable(np.zeros(w_shape), name="weights")
    ...
    cost = tf.divide(-tf.reduce_sum(y*tf.log(pred)+(1-y)*tf.log(1-pred)),num_train)
    #cost = tf.reduce_mean(-tf.reduce_sum(y*tf.log(pred)+(1-y)*tf.log(1-pred)),reduction_indices=[1])
    #cost = -(tf.matmul(tf.transpose(y),tf.log(pred))+tf.matmul(tf.transpose(1-y),tf.log(1-pred)))/num_train
...
#    optimizer = tf.train.MomentumOptimizer(learning_rate,momentum_rate).minimize(cost) 
